...
def write_output(output_file: Path, merged_results: BacktestResult) -> None:
    ...
    output = {}
    output["submissionId"] = str(uuid.uuid4())
    output["activitiesLog"] = "day;timestamp;product;bid_price_1;bid_volume_1;bid_price_2;bid_volume_2;bid_price_3;bid_volume_3;ask_price_1;ask_volume_1;ask_price_2;ask_volume_2;ask_price_3;ask_volume_3;mid_price;profit_and_loss"
    output["activitiesLog"] += "\n" + "\n".join(str(row) for row in merged_results.sandbox_logs) + "\n"
    output["logs"] = str(merged_results.activity_logs)
    output["tradeHistory"] = str(merged_results.trades)
    with output_file.open("w+", encoding="utf-8") as file:
        # json.dump(output, file)
        id = str(uuid.uuid4())
        file.write("{")
        file.write(f"\"submissionId\":\"{id}\",")
        ...
        file.write("],\n\n\n\"tradeHistory\":\n")
        file.write("[\n")
        file.write(str(merged_results.trades[0]))
...
def cli(
    ...
    data: Annotated[Optional[Path], Option(help="Path to data directory. Must look similar in structure to https://github.com/mr_rambling/imc-prosperity-4-backtester/tree/master/prosperity3bt/resources.", show_default=False, exists=True, file_okay=False, dir_okay=True, resolve_path=True)] = None,
    ...
    if len(parsed_days) > 1:
        print_overall_summary(results, True)
